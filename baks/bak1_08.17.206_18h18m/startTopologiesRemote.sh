...
deploy_topology_fn() {
    TPLGY_NAME=$1
    ssh_exec_fn "cd $STORM_HOME_CLUSTER; $STORM_HOME_CLUSTER/bin/storm jar $STORM_HOME_CLUSTER/examples/storm-starter/storm-starter-topologies-0.10.0-SNAPSHOT.jar storm.starter.WordCountTopology $TPLGY_NAME -c topology.workers=1 -c topology.debug=false"
#    sleep $SECS
}

kill_topology_fn() {
    TPLGY_NAME=$1
    ssh_exec_fn "cd $STORM_HOME_CLUSTER; $STORM_HOME_CLUSTER/bin/storm kill $TPLGY_NAME"
#    sleep $SECS
}
